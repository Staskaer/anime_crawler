...
import math
import hashlib
from .bloom_utils import range_fn, is_string_io, running_python_3
from struct import unpack, pack, calcsize
from anime_crawler.settings import BLOOM_FILTER_CAPACITY, BLOOM_FILTER_ERROR
...
class _BloomFilter(object):
    ...
    def copy(self):
        ...
        """
        new_filter = _BloomFilter(self.capacity, self.error_rate)
...
class BloomFilter:
    def __init__(self) -> None:
        '''
        构造函数，采用多个布隆过滤器级联的方式进行过滤
        '''
        self.bloom_filter_list = [_BloomFilter(
            BLOOM_FILTER_CAPACITY, BLOOM_FILTER_ERROR)]
        self.current_bloom_filter = 0
        self.nums = 0
    ...
    def add(self, key: str) -> bool:
        ...
        Returns:
            bool: 返回是否添加成功
        '''
        if self.nums >= BLOOM_FILTER_CAPACITY:
            self.nums = 0
            self.bloom_filter_list.append(_BloomFilter(
                BLOOM_FILTER_CAPACITY, BLOOM_FILTER_ERROR))
            self.current_bloom_filter += 1

        self.nums += 1
        return not self.bloom_filter_list[self.current_bloom_filter].add(key)
    ...
    def find(self, key: str) -> bool:
        ...
        Returns:
            bool: 是否找到
        '''
        for filter_ in self.bloom_filter_list:
            if key in filter_:
                return True
        return False

    # TODO 持久化
    def to_file(self, file: str) -> bool:
        ...
